...
static PING_SEM: Semaphore = Semaphore::new(0, 1);
static PONG_SEM: Semaphore = Semaphore::new(0, 1);

const MAX_ITERS: u32 = 1000;

extern "C" fn task_ping() {
    defmt::info!("=== BENCHMARK: Preemptive Context Switch ===");
...
fn main() -> ! {
    ...
    whyos::TaskBuilder::new(task_ping).priority(5).name("Ping").spawn().unwrap();
