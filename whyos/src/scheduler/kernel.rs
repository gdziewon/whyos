use crate::{ResumeContext, TaskState, error::{WhyError, WhyResult}, task::{Stack, TaskId, TaskMap, TaskTable, Tcb}};
...
impl Kernel {
    ...
    pub fn current_task(&self) -> TaskId { self.current_task }
    ...
    pub fn remove_zombie(&mut self, tid: TaskId) -> Option<Stack> {
    ...
    fn pick_next(&self) -> TaskId {
        // start searching from (current + 1) for round robin
        let next = (self.current_task.id() + 1) % MAX_TASKS;
